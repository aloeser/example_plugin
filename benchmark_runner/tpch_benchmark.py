...
class TPCHBenchmark(AbstractBenchmark):
  ...
  def sort_orders(self):
    ...
    # one more final SF 10 2d TOp 20 correction
    return {
 ...
 '13-l_suppkey-31_l_orderkey-31': {'lineitem': [('l_suppkey', 31),
   ('l_orderkey', 31)]},
    ...
    return {
      'l_orderkey-100': {
        'lineitem': [['l_orderkey', 100]]
      },
      'l_shipdate-20_l_orderkey-50': {
        'lineitem': [['l_shipdate', 20], ['l_orderkey', 50]]
      },
      'l_partkey-100': {
        'lineitem': [['l_partkey', 100]]
      },
    }

    # final TOP 20 SF 10 lineitem clusterings
    return {
    '00-l_shipdate-100': {'lineitem': [('l_shipdate', 100)]},
